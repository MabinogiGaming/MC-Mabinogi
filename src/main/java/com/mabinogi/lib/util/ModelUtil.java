...
public class ModelUtil {
	...
	public static ImmutableList<BakedQuad> getQuadsForSprite(int tint, int color, TextureAtlasSprite sprite, VertexFormat format, Optional<TRSRTransformation> transform)
    {
        ...
        for (EnumFacing facing : HORIZONTALS)
        {
            for (int v = 0; v < vMax; v++)
            {
                ...
                for (int u = 0; u < uMax; u++)
                {
                    ...
                    else
                    {
                        if (building && !face) // finish current quad
                        {
                            ...
                            builder.add(buildSideQuad(format, transform, facing, tint, color, sprite, uStart, v+off, u-uStart));
                            building = false;
                        }
                ...
                if (building) // build remaining quad
                {
                    ...
                    int off = facing == EnumFacing.DOWN ? 1 : 0;
                    builder.add(buildSideQuad(format, transform, facing, tint, color, sprite, uStart, v+off, uEnd-uStart));
                }
            }
        }
        ...
        for (EnumFacing facing : VERTICALS)
        {
            for (int u = 0; u < uMax; u++)
            {
                ...
                for (int v = 0; v < vMax; v++)
                {
                    ...
                    else
                    {
                        if (building && !face) // finish current quad
                        {
                            ...
                            builder.add(buildSideQuad(format, transform, facing, tint, color, sprite, u+off, vStart, v-vStart));
                            building = false;
                        }
                        else if (!building && face) // start new quad
                        {
                ...
                if (building) // build remaining quad
                {
                    ...
                    int off = facing == EnumFacing.EAST ? 1 : 0;
                    builder.add(buildSideQuad(format, transform, facing, tint, color, sprite, u+off, vStart, vEnd-vStart));
                }
            }
        }
    ...
    private static BakedQuad buildSideQuad(VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, int tint, int color, TextureAtlasSprite sprite, int u, int v, int size)
    {
        ...
        return buildQuad(
            format, transform, remap(side), sprite, tint, color,
    ...
    private static BakedQuad buildQuad(VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, TextureAtlasSprite sprite, int tint, int color,
